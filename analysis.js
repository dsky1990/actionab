...
Object.keys(finalData).forEach(key => {
  ...
  Object.keys(finalInData).forEach(keyin => {
    ...
    if(len > 0) {
      ...
      console.log(`${key}-${keyin}开单${len}次，预测成功${time}次， 准确率${Number(time/len*100).toFixed(2)}%，盈利${finalMoney[keyin]}`)
    } 
  })
})
Object.keys(mcData).forEach(key => {
  ...
  Object.keys(mcInData).forEach(keyin => {
    ...
    if(len > 0) {
      ...
      console.log(`${key}-${keyin}开单${len}次，预测成功${time}次， 准确率${Number(time/len*100).toFixed(2)}%，盈利${mcMoney[keyin]}`)
    } 
  })
})
